...
### 前端 UI 打磨（P6-4-C）
统一的**两层 UI 令牌**已落地（`client/src/theme.ts` 基础令牌 + `.zf-*` 组件类，`tokens.css` 限前缀作用域，不用全局标签选择器）：
- 交互态全覆盖：hover/active/focus-visible（2px 强调描边）/disabled（0.5 透明+灰度）/表单错误成功/列表选中
- WCAG-AA 对比度：正文 ≥4.5:1、辅助 ≥3:1；治理面板并入浅色系统，消除双配色分裂
